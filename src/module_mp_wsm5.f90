...
! print *,"ifndef XEON_OPTIMIZED_WSM5 OK"
	  ...
	  j=jts ! BLOCK 1 - 0.7280159 !! DESTA FORMA GPU JA ESTA PIOR QUE CPU :-(
         ...
         CALL wsm52D(th(1,kts:kte,j), pii(1,kts:kte,j), q(ims,kms,j)&
					,qc(1,kts:kte,j),qi(1,kts:kte,j), qr(1,kts:kte,j),qs(1,kts:kte,j) &
                    ,den(ims,kms,j)                                &
                    ,p(ims,kms,j), delz(ims,kms,j)                 &	! ANOTAR QUAIS SAO PRIVADAS
                    ,delt,g, cpd, cpv, rd, rv, t0c                 &
                    ,ep1, ep2, qmin                                &
                    ,XLS, XLV0, XLF0, den0, denr                   &
                    ,cliq,cice,psat                                &
...
! th(i,k,j)=t(i,k)/pii(i,k,j)
!th, pii
  SUBROUTINE wsm52D(th, pii, q                                          & 
                   ,qc, qi, qr, qs, den, p, delz                        &
                   ,delt,g, cpd, cpv, rd, rv, t0c                 &
                   ,ep1, ep2, qmin                                &
                   ,XLS, XLV0, XLF0, den0, denr                   &
                   ,cliq,cice,psat                                &
...
!
      ! cpmcal(x) = cpd*(1.-max(x,qmin))+max(x,qmin)*cpv DESCOMENTAR
...
! BLOCK 1 !0.8056290
      ! do k = kts, kte
        ! do i = its, ite
		  ...
		  !t(i,k)=th(i,k)*pii(i,k) !10/12/2023
        ! enddo
      ! enddo
! /BLOCK 1
!----------------------------------------------------------------
...
!
      ! loops = max(nint(delt/dtcldcr),1) DESCOMENTAR
      ! dtcld = delt/loops
...
!!         qss(i,k,2) = fpvs(t(i,k),1,rd,rv,cpv,cliq,cice,xlv0,xls,psat,t0c)
      ! hsub = xls DESCOMENTAR DAQUI PRA BAIXO
      ! hvap = xlv0
      ! cvap = cpv
      ! ttp=t0c+0.01
      ! dldt=cvap-cliq
...
!BLOCK 7 !0.8629489 
      ! do k = kts, kte
        ! do i = its, ite
          ! tr=ttp/t(i,k)
          ! logtr=log(tr)
          ! qss(i,k,1)=psat*exp(logtr*(xa)+xb*(1.-tr))
          ! qss(i,k,1) = min(qss(i,k,1),0.99*p(i,k))
          ! qss(i,k,1) = ep2 * qss(i,k,1) / (p(i,k) - qss(i,k,1))
          ! qss(i,k,1) = max(qss(i,k,1),qmin)
          ! rh(i,k,1) = max(q(i,k) / qss(i,k,1),qmin)
          ! if(t(i,k).lt.ttp) then
            ! qss(i,k,2)=psat*exp(logtr*(xai)+xbi*(1.-tr))
          ! else
            ! qss(i,k,2)=psat*exp(logtr*(xa)+xb*(1.-tr))
          ! endif
          ! qss(i,k,2) = min(qss(i,k,2),0.99*p(i,k))
          ! qss(i,k,2) = ep2 * qss(i,k,2) / (p(i,k) - qss(i,k,2))
          ! qss(i,k,2) = max(qss(i,k,2),qmin)
          ! rh(i,k,2) = max(q(i,k) / qss(i,k,2),qmin)
        ! enddo
      ! enddo
!/BLOCK 7
!----------------------------------------------------------------
      ...
      ! do k = kts, kte
        ! do i = its, ite
          ! work1(i,k,1) = ((((den(i,k))*(xl(i,k))*(xl(i,k)))*((t(i,k))+120.)    &
                      ...
                      ! +  p(i,k)/((qss(i,k,1))*(8.794e-5*exp(log(t(i,k))*(1.81))))
          ! work1(i,k,2) = ((((den(i,k))*(xls)*(xls))*((t(i,k))+120.)*(den(i,k)))&
                       ! /(1.414e3*(1.496e-6*((t(i,k))*sqrt(t(i,k))))*(den(i,k)) &
                       ! *(rv*(t(i,k))*(t(i,k))))                                &
                      ! + p(i,k)/(qss(i,k,2)*(8.794e-5*exp(log(t(i,k))*(1.81)))))
...
!BLOCK 24 !0.8380830
      ! do k = kts, kte
        ! do i = its, ite
          ! supsat = max(q(i,k),qmin)-qss(i,k,1)
          ! satdt = supsat/dtcld
!---------------------------------------------------------------
...
!BLOCK 25 !0.8388951
      ...
      ! do k = kts, kte
        ! do i = its, ite
          ! supcol = t0c-t(i,k)
          ! n0sfac(i,k) = max(min(exp(alpha*supcol),n0smax/n0s),1.)
          ! supsat = max(q(i,k),qmin)-qss(i,k,2)
          ! satdt = supsat/dtcld
          ! ifsat = 0
!-------------------------------------------------------------
! Ni: ice crystal number concentraiton   [HDC 5c]
...
!BLOCK 28 !0.8564219
      ! do k = kts, kte
        ! do i = its, ite
          ! work1(i,k,1) = ((max(q(i,k),qmin)-(qss(i,k,1)))/(1.+(xl(i,k))         &   
                        ! *(xl(i,k))/(rv*(cpm(i,k)))*(qss(i,k,1))                 &
                        ! /((t(i,k))*(t(i,k)))))

...
CONTAINS
  ...
  COMPLEX FUNCTION m_complex_water_ray(lambda,T)  
  ...
  subroutine rayleigh_soak_wetgraupel (x_g, a_geo, b_geo, fmelt,    &
    ...
    complex :: teste_var
    real :: teste_a, teste_b
    
    !     refractive index of air:
    !m_air = (1.0d0,0.0d0)
    teste_a = 1.0
    teste_b = 0.0
    m_air = cmplx(teste_a, teste_b)
    ...
    if (D_g .ge. 1d-12) then
       ...
       !teste_var = cmplx(teste_a, teste_b)
       
       !call get_m_mix_nested_sub2(m_air, m_i, m_w)
       !(m_air, m_i, m_w)
...
!!$
!!$      subroutine get_m_mix_nested_sub2(m_a, m_i, m_w)
!!$
!!$        !$acc routine seq
!!$
!!$        implicit none
...
!!$
!!$      end subroutine get_m_mix_nested_sub2
!!$      
  subroutine get_m_mix_nested_sub(m_a, m_i, m_w, volair,      &
    ...
    complex, intent(out) :: get_m_mix_nested
